...
export default function TheBulletin({ analyses, onSelectArticle, selectedId }: BulletinProps) {
  return (
    <div className="border-4 border-white bg-black h-full flex flex-col">
      ...
      <div className="border-b-4 border-white px-4 py-2 flex items-center justify-between">
        <h2 className="text-sm font-bold tracking-widest text-white">THE BULLETIN</h2>
        <div className="flex items-center gap-2">
          <div className="w-2 h-2 bg-[#FF0000] animate-pulse" />
          <span className="text-xs text-gray-400">{analyses.length} ITEMS</span>
        </div>
      </div>
            ...
            return (
              ...
              >
                ...
                <p className={`text-sm font-bold leading-tight mb-1 ${
                  isLethal ? 'text-[#FF0000]' : isSell ? 'text-[#FF6666]' : 'text-white'
                }`}>
                  {item.original.headline}
                </p>

                {/* Summary */}
                <p className="text-xs text-gray-400 leading-snug">{item.summary}</p>

                {/* Footer: source + affected assets */}
                <div className="flex items-center justify-between mt-2">
                  <span className="text-[10px] text-gray-600">{item.original.source}</span>
...
function SentimentTag({ sentiment }: { sentiment: string }) {
  ...
  return (
    <span className={`text-[9px] font-bold px-1 py-0.5 border ${colors[sentiment] || 'text-gray-400 border-gray-600'}`}>
      {sentiment}
    </span>
  );
}
...
function ActionTag({ action }: { action: string }) {
  ...
  return (
    <span className={`text-[9px] font-bold px-1.5 py-0.5 ${colors[action] || 'bg-gray-800 text-gray-400'}`}>
      {action}
    </span>
  );
}
