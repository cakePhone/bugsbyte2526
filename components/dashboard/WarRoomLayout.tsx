  ...
  return (
    <WarRoomProvider>
      <div className="min-h-screen bg-[#121212] text-white font-mono flex flex-col">
        {/* Command Header */}
        <CommandHeader scanCount={scanCount} isScanning={isScanning} availableAssets={availableAssets} />

        {/* Main Content Area */}
        <main className="flex-1 flex pb-10">
          {children}
        </main>

        {/* War Room Footer with World Clock */}
        <WarRoomFooter connectionStatus={isScanning ? "reconnecting" : "connected"} />
      </div>
    </WarRoomProvider>
  );
}
