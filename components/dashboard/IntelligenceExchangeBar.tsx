...
interface IntelligenceExchangeBarProps {
  ...
  isLoading?: boolean;
  apiFailed?: boolean;
  onExecuteTrade?: (exchange: string, symbol: string) => void;
}
...
export default function IntelligenceExchangeBar({
  ...
  const sortedQuotes = [...quotes].sort((a, b) => a.netPriceAfterTax - b.netPriceAfterTax);
  ...
  const getSpeedLabel = (speed: ExchangeQuote["executionSpeed"]) => {
    switch (speed) {
      ...
      case "MEDIUM": return "< 500ms";
      case "SLOW": return "> 1s";
    }
  };

  const handleExecute = (exchange: string, symbol: string) => {
    onExecuteTrade?.(exchange, symbol);
    // Exit buy mode after execution
    setBuyMode(false, null);
  };

  return (
    <div className={`border-4 bg-black ${apiFailed ? "border-[#FF0000] animate-pulse" : isBuyMode ? "border-[#00FF88]" : "border-white"}`}>
      {/* Header */}
      <div className={`border-b-4 px-4 py-2 flex items-center justify-between ${isBuyMode ? "border-[#00FF88] bg-[#00FF88]/10" : "border-white"}`}>
        <div className="flex items-center gap-3">
          <h3 className="text-sm font-black font-mono uppercase tracking-widest text-white">
            {isBuyMode ? "BUY ORDER — SELECT SELLER" : "INTELLIGENCE EXCHANGE BAR"}
          </h3>
          {isBuyMode && buySymbol && (
        ...
        <div className="flex min-w-max">
          {sortedQuotes.length === 0 ? (
            <div className="px-6 py-8 text-sm font-mono text-gray-600 uppercase">
              {isBuyMode ? "NO SELLERS AVAILABLE FOR THIS ASSET..." : "NO EXCHANGE DATA AVAILABLE..."}
            </div>
          ) : (
            sortedQuotes.map((quote, index) => {
              ...
              return (
                ...
                >
                  ...
                  {isGreenBean && (
                    <>
                      <div className={`absolute top-0 left-0 w-0 h-0 border-l-[20px] border-t-[20px] border-r-[20px] border-b-[20px] border-r-transparent border-b-transparent ${
                        isBuyMode ? "border-l-[#00FF88] border-t-[#00FF88]" : "border-l-[#FF0000] border-t-[#FF0000]"
                      }`} />
                  ...
                  <div className="absolute top-2 right-2">
                    <span className={`text-[10px] font-black font-mono ${
                      isGreenBean ? (isBuyMode ? "text-[#00FF88]" : "text-[#FF0000]") : "text-gray-600"
                    }`}>
                      RANK {rank}
                    </span>
                  </div>
                  ...
                  <div className="mb-3 mt-1">
                    ...
                    {isGreenBean && (
                      <span className={`ml-2 text-[8px] font-black font-mono px-1 py-0.5 border ${
                        isBuyMode 
                  ...
                  <div className="border-t-2 border-gray-800 pt-2 mb-2">
                    ...
                    <div className={`text-xl font-black font-mono ${
                      ...
                      ${quote.netPriceAfterTax.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                    </div>
                  </div>
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-[9px] font-mono">
                      ...
                      <span className="text-gray-400">${quote.askPrice.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                    </div>
                    <div className="flex justify-between text-[9px] font-mono">
                      <span className="text-gray-600">TAX</span>
                      <span className="text-gray-400">{quote.taxRate.toFixed(2)}%</span>
                    </div>
                    <div className="flex justify-between text-[9px] font-mono">
                      <span className="text-gray-600">WITHDRAWAL</span>
                      <span className="text-gray-400">${quote.withdrawalFee.toFixed(2)}</span>
                    </div>
                  </div>
                  ...
                  <div className="border-t-2 border-gray-800 pt-2 mt-2">
                    ...
                    <div className={`text-sm font-black font-mono ${getSpeedColor(quote.executionSpeed)}`}>
                      {quote.executionSpeed} <span className="text-[10px] text-gray-500">{getSpeedLabel(quote.executionSpeed)}</span>
                    </div>
                  </div>
                  ...
                  {isBuyMode && (
                    <div className="border-t-2 border-gray-800 pt-2 mt-2">
                      <div className={`text-center text-xs font-black font-mono uppercase py-1 ${
 ...
 */
export function generateMockExchangeQuotes(symbol: string, basePrice: number): ExchangeQuote[] {
  const exchanges = [
    { name: "UPHOLD", taxRate: 0.15, withdrawalFee: 1.50, speed: "FAST" as const, priceVar: 0 },
    { name: "COINBASE", taxRate: 0.25, withdrawalFee: 2.00, speed: "MEDIUM" as const, priceVar: 0.002 },
    { name: "KRAKEN", taxRate: 0.16, withdrawalFee: 1.00, speed: "FAST" as const, priceVar: -0.001 },
    { name: "BINANCE", taxRate: 0.10, withdrawalFee: 0.50, speed: "FAST" as const, priceVar: -0.003 },
    { name: "GEMINI", taxRate: 0.35, withdrawalFee: 3.00, speed: "SLOW" as const, priceVar: 0.005 },
  ];

  return exchanges.map((ex, index) => {
    const askPrice = basePrice * (1 + ex.priceVar + Math.random() * 0.002);
    const bidPrice = askPrice * 0.998;
    const netPriceAfterTax = askPrice * (1 + ex.taxRate / 100) + ex.withdrawalFee;
