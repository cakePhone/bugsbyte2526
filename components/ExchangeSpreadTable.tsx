...
export default function ExchangeSpreadTable({ opportunities }: Props) {
  if (!opportunities.length) {
    return (
      <div className="border-4 border-white bg-black p-6 font-mono text-center text-gray-400 text-sm uppercase">
        NO EXCHANGE DATA
      </div>
    );
  }
    ...
    <div className="border-4 border-white bg-black text-white">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left">
          <thead>
            <tr className="border-b-4 border-white text-[10px] font-mono font-black uppercase text-gray-400">
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-2 py-2 text-center">
                      {isGreenBean ? (
                        <span className="text-[10px] font-black font-mono bg-red-600 text-white px-2 py-0.5 border-2 border-white inline-block">
                          ☕ GREEN BEAN
                        </span>
                      ) : (
      ...
      {opportunities.map((opp) => (
        <div
          ...
          className="border-t-4 border-white px-4 py-2 flex items-center justify-between bg-black"
        >
          <span className="font-mono font-black text-xs">
            {opp.symbol} ARBITRAGE: +{opp.potentialProfitPct.toFixed(3)}%
          </span>
          <span
            className={`text-[10px] font-black font-mono px-2 py-0.5 border-2 border-white ${
