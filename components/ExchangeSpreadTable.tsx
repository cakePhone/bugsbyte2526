...
export default function ExchangeSpreadTable({ opportunities }: Props) {
  if (!opportunities.length) {
    return (
      <div className="border-4 border-black bg-white p-6 font-mono text-center text-gray-400 text-sm uppercase">
        NO EXCHANGE DATA
      </div>
    );
  }
  ...
  return (
    <div className="border-4 border-black bg-white">
      <div className="border-b-4 border-black px-4 py-2 flex items-center justify-between">
        <h2 className="text-sm font-black font-mono uppercase">EXCHANGE SPREADS</h2>
        <span className="text-[10px] font-mono font-bold">3 FEEDS</span>
      </div>
                ...
                return (
                  <motion.tr
                    ...
                    className={`border-b border-black ${
                      isGreenBean ? 'bg-red-50' : 'hover:bg-gray-50'
                    }`}
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    transition={{ duration: 0.2 }}
                  >
                    <td className="px-4 py-2 font-mono font-black text-sm">{opp.symbol}</td>
                    <td className="px-2 py-2 font-mono text-sm font-bold">{q.exchange}</td>
                    <td className="px-2 py-2 text-right font-mono text-sm">
                      ${q.ask.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                    </td>
                    <td className="px-2 py-2 text-right font-mono text-sm">
                      ${q.bid.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                    </td>
                    <td className="px-2 py-2 text-right font-mono text-sm">
                    ...
                    <td className="px-2 py-2 text-center">
                      {isGreenBean ? (
                        <span className="text-[10px] font-black font-mono bg-red-600 text-white px-2 py-0.5 border-2 border-black inline-block">
                          ☕ GREEN BEAN
                        </span>
                      ) : (
      ...
      {opportunities.map((opp) => (
        <div
          ...
          className="border-t-4 border-black px-4 py-2 flex items-center justify-between bg-gray-50"
        >
          <span className="font-mono font-black text-xs">
            {opp.symbol} ARBITRAGE: +{opp.potentialProfitPct.toFixed(3)}%
          </span>
          <span
            className={`text-[10px] font-black font-mono px-2 py-0.5 border-2 border-black ${
